...
function App() {
  ...
  const handleAskQuestion = async () => {
    ...
    try {
      ...
      // console.error(err);
      setResult("Error occurred while fetching response");
    }
  };

  const handleIsEnter = (e) => {
    if (e.key === 'Enter') {
      handleAskQuestion();
    }
  }
  ...
  return (
    <div className={darkMode === 'dark' ? 'dark' : 'light'}>
      <div className='grid grid-cols-5 h-screen text-center'>
        <select onChange={(e) => setDarkMode(e.target.value)} className='fixed dark:text-white text-zinc-800 dark: dark:bg-zinc-800
         bg-red-100 bottom-0 p-5'>
        ...
        <div className='col-span-4 p-10'>
          ...
          <div ref={scrollToAns} className='container h-110 overflow-auto scrollbar-hide no-scrollbar'>
            <div className='dark:text-zinc-300 text-zinc-800'>
              <ul>
                {
                  result.map((item, index) => (
                    <QuestionsAnswer key={index} item={item} index={index} />
                  ))
                }
              </ul>
            </div>
          </div>
          <div className='dark:bg-zinc-800 bg-red-100 w-1/2 p-1 pr-5 dark:text-white text-zinc-800 m-auto rounded-4xl border border-zinc-600 flex h-16'>
            <input type='text'
              onKeyDown={handleIsEnter}
              value={question}
              onChange={(e) => setQuestion(e.target.value)}
              className='w-full h-full p-3 outline-none'
              placeholder='Ask me anything' />
            <button className='cursor-pointer' onClick={handleAskQuestion}>Ask</button>
          </div>
        </div>
      </div>
    </div>
